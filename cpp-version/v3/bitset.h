...
template<>
struct _Bitset<128> {
	constexpr static int capacity = 128;
	typedef uint64_t BlockType;
	BlockType d0 = 0;
	BlockType d1 = 0;

	_Bitset operator &(const _Bitset& b) const {
	...
	_Bitset& join(const _Bitset& b) {
		d0 |= b.d0;
		d1 |= b.d1;
		return *this;
	}

	_Bitset& substract(const _Bitset& b) {
		d0 &= ~b.d0;
		d1 &= ~b.d1;
		return *this;
	}
...
template<> struct std::hash<_Bitset<64>> {
...
template<>
struct _Bitset<32> {
	constexpr static int capacity = 32;
	...
	_BitsetIterator<32> begin() const {
		return _BitsetIterator<32>{*this, ffs()};
	};

	_BitsetIterator<32> end() const {
		return _BitsetIterator<32>{*this, 0};
	};
};

bool operator == (const _Bitset<32> s0, const _Bitset<32> s1) {
	return s0.d0 == s1.d0;
}

template<> struct std::hash<_Bitset<32>> {
	std::size_t operator()(_Bitset<32> const& s) const noexcept {
		return (s.d0 * 0xcc9e2d51);
	}
};
